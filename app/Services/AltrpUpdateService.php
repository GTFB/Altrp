...
class AltrpUpdateService
{
  ...
  public function update( $test = false)
  {
    ...
    \Log::info(date('d.m.Y H:i:s') . " - Get Last Version | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    ...
    \Log::info(date('d.m.Y H:i:s') . " - Download Last Version | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);

    if ( ! $this->write_public_permissions() ) {
      throw new \HttpException( 'Не удалось обновить режим чтения файлов' );
    }

    \Log::info(date('d.m.Y H:i:s') . " - Write Public Permissions | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);

    if ( ! $this->save_archive( $file ) ) {
      throw new \HttpException( 'Не удалось сохранить архив' );
    }

    \Log::info(date('d.m.Y H:i:s') . " - Saved Archive | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);

    if ( ! $this->update_files() ) {
      throw new \HttpException( 'Не удалось обновить файлы' );
    }
    \Log::info(date('d.m.Y H:i:s') . " - Updated Files | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    ...
    \Log::info(date('d.m.Y H:i:s') . " - Deleted Archive File | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    if( File::exists( base_path( 'routes/page_routes.php' ) ) ){
      File::delete( base_path( 'routes/page_routes.php') );
    }
    \Log::info(date('d.m.Y H:i:s') . " - Deleted Page Routes | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    /**
     * Проверяем нужно ли производить агрейд
     */
    $this->checkVersion( $version );
    \Log::info(date('d.m.Y H:i:s') . " - Check Version | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);

    // Lunch the installation if the /.env file doesn't exists
    if ( ! File::exists( base_path( '.env' ) ) ) {
      \Log::info(date('d.m.Y H:i:s') . " - Missing File .env - Go To Install | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    ...
    // Clear all the cache
    $this->clearCache();
    \Log::info(date('d.m.Y H:i:s') . " - Cache Cleared  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    ...
    \Log::info(date('d.m.Y H:i:s') . " - Upgraded Database  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    // Write providers
    $this->writeProviders();
    \Log::info(date('d.m.Y H:i:s') . " - Written Providers  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    // Update modules statuses
    $this->updateModulesStatuses();
    \Log::info(date('d.m.Y H:i:s') . " - Updated Module Statuses  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    // Update the current version to last version
    $this->setCurrentVersion( $version );
    \Log::info(date('d.m.Y H:i:s') . " - Set Current Version  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    // Clear all the cache
    $this->clearCache();
    \Log::info(date('d.m.Y H:i:s') . " - Cache Cleared  | User Ip: " . $_SERVER['REMOTE_ADDR'] . " | Method:" . __METHOD__);
    return true;
