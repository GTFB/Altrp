...
export default function renderAccordion(settings, device) {
  ...
  if(settings.active_icon_accordion_content) {
    active_icon = renderAsset(settings.active_icon_accordion_content, {class: "altrp-accordion-item-active-icon-svg"});
  }
  const title_html_tag_accordion_content = getResponsiveSetting(settings,'title_html_tag_accordion_content', device) || 'div'

  return `<div class="altrp-accordion">
  ${items.map((item, idx) => {
    return (
      `<div class="${"altrp-accordion-item" + (activeItem[idx] ? ' active' : '')}">
