...
class Import extends Component {
  constructor(props) {
    ...
    this.custom_resource = new Resource({
      route: "/admin/ajax/import/custom_settings"
    });
  }
  ...
  onSubmit = async e => {
    e.preventDefault();
    let res = await this.resource.postFiles(
      this.inputFile.current.files,
  ...
  onCustomSubmit = async e => {
    e.preventDefault();
    let res = await this.custom_resource.postFiles(
      this.inputCustomFile.current.files,
      "application/zip,application/x-zip,application/x-zip-compressed"
    );
    if (res.success) {
      await alert("Success");
