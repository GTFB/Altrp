...
class IterationElement {
  ...
  constructor(element) {
    ...
    if (informers && !element.hasAttribute('data-react-element')) {
      this.processInformers(informers)
    }
  ...
  processInformers(informers) {
    const container = document.createElement('div')
    container.classList.add('altrp-informers')
    const elementId = this.element.dataset?.altrpId || ''
    this.element.appendChild(container)
    document.addEventListener('altrp-store-updates', function () {
      import(/* webpackChunkName: 'Informer' */'../components/Informer').then(Informer => {
        Informer = Informer.default
        window.ReactDOM.render(<div className='altrp-informers'>
          {informers.map((item, idx) => {
            return <Informer
              {...item}
              key={`informer_${elementId}_${idx}`}/>
          })}
        </div>, container, () => {
        })
      })
