...
export default class ModelsController {
  async index({ response, request }: HttpContextContract) {
    ...
    await Promise.all(
      modelToUpdate.map(async (m: Model) => {
        m.guid = guid();
        await m.save();
    ...
    if (!model) {
      ...
      if (model.table.name !== string.pluralize(modelData.name)) {
        ...
        await client.schema.renameTable(
          `${model.table.name}`,
          `${string.pluralize(modelData.name)}`
        );
      ...
      if (modelData.time_stamps) {
        const created_at_column_exists = await Column.query()
          .where('name', '=', 'created_at')
          .andWhere('model_id', '=', model.id);
        const updated_at_column_exists = await Column.query()
        ...
        const created_at_column = await Column.query()
          .where('name', '=', 'created_at')
          .andWhere('model_id', '=', model.id);
        const updated_at_column = await Column.query()
          .where('name', '=', 'updated_at')
          .andWhere('model_id', '=', model.id);
      ...
      if (modelData.soft_deletes) {
        const deleted_at_column_exists = await Column.query()
      ...
      if (!empty(request.all().categories)) {
        await Promise.all(
          request.all().categories.map(async (c) => {
  ...
  async getModelRelations({ response, params }: HttpContextContract) {
    const model = await Model.query().where('id', params.id).preload('altrp_relationships').first();
    if (!model) {
      response.status(404);
      return response.json({
        success: false,
  ...
  async checkRelationName({ response, request, params }: HttpContextContract) {
    ...
    return response.json({
      taken: !(await Relationship.query()
        .where({
          model_id: model.id,
  ...
  async storeModel({ request, response, auth }: HttpContextContract) {
    ...
    if (!empty(modelData.categories)) {
      await Promise.all(
        modelData.categories.map(async (c) => {
          let newCatObj = new CategoryObject();
          newCatObj.fill({
            category_guid: c.value,
            object_guid: model.guid,
    ...
    if (relationship) {
      ...
      for (let i in relationship) {
        ...
        for (let j in relations) {
          if (
            relationship[i].type != 'belongsTo' &&
            relations[j] &&
            relationship[i].add_belong_to
          ) {
            try {
              //@ts-ignore
  ...
  async getModelFieldOptions({ params, response }) {
    ...
    return response.json({
      options: model?.table.columns.map((c) => ({ value: c.id, label: c.title })) || [],
    });
  }
  ...
  async showDataSource({ response, params }: HttpContextContract) {
    let dataSource = await Source.query()
      .where('id', params.id)
      .preload('roles')
      .preload('permissions')
      .first();
  ...
  async updateDataSource({ response, request, params }: HttpContextContract) {
    const dataSourceSchema = schema.create({
      ...
      name: schema.string({ trim: true }, [rules.maxLength(32)]),
    });
          ...
          if (data.access) {
            if (data.access.roles) {
              ...
              if (data.access.roles.length > 0) {
                await dataSource
                  .related('roles')
                  .attach(data.access.roles.map((role) => role.value));
              }
            }
          }
        default:
  ...
  async getDataSourcesByModel({ response, params }: HttpContextContract) {
    let data_sources = await Source.query()
