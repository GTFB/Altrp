...
const requestSiteName = new Resource({
...
export default class AdminSettings extends Component {
  constructor(props) {
    ...
    this.siteNameHandler = this.siteNameHandler.bind(this);
    this.changeSiteName = this.changeSiteName.bind(this);
  }

  async componentDidMount() {
    const req = await requestSiteName.get("site_name");
    this.setState(s => ({ ...s, siteName: req?.site_name || "" }));
  }
   ...
   * @param {Event} e
   */
  siteNameHandler = e => this.changeSiteName(e.target.value);

  changeSiteName = _.debounce(async name => {
    const req = await requestSiteName.put("site_name", { value: name });
    if (req.result == true) {
      this.setState(s => ({ ...s, siteName: name }));
    }
  }, 100);
                  ...
                  <tr className="admin-settings-table-row">
                    ...
                    <td className="admin-settings-table__td ">
                      <input
                        ...
                        type="checkbox"
                      />
                      Checking this box will disable Builder's Default Colors,
                      and make Builder inherit the colors from your CSS file
                    </td>
                  </tr>
                  <tr className="admin-settings-table-row">
                    <td className="admin-settings-table__td row-text">
