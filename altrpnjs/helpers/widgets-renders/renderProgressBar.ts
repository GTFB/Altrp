...
export default function renderProgressBar(settings, device, context) {
  ...
  return `<div aria-valuemax="100" aria-valuemin="0" aria-valuenow="${value}" class="${"bp3-progress-bar" + (animate ? "" : " bp3-no-animation") + (stripes ? "" : " bp3-no-stripes")}" role="progressbar"><div class="bp3-progress-meter" style="${"width: " + value + "%"}"></div></div>`
}
