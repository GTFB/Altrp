...
export default class Customizer extends BaseModel {
  ...
  public static parseData(data, customizer) {
    data = data.map((item) => {
      ...
      switch (type) {
        case 'default':
          return new Edge(item, customizer);
        case 'switch':
          return new SwitchNode(item, customizer);
        case 'start':
          return new StartNode(item, customizer);
        case 'return':
          return new ReturnNode(item, customizer);
        case 'change':
          return new ChangeNode(item, customizer);
        case 'documentAction':
          return new DocumentNode(item, customizer);
        case 'crudAction':
          return new CrudNode(item, customizer);
        case 'apiAction':
          return new ApiNode(item, customizer);
        case 'messageAction':
          return new MessageNode(item, customizer);
        case 'customizer':
          return new CustomizerNode(item, customizer);
        case 'discordAction':
          return new DiscordNode(item, customizer);
        default:
  ...
  renderForController(): string {
    const data = this.getStartNode()?.data?.data || {};

    let afterMethod = '';

    if (data.static_method) {
      afterMethod += 'static ';
    }

    if (data.async_method) {
      afterMethod += 'async ';
    }

    return `
  ${afterMethod}${this.name}() {
