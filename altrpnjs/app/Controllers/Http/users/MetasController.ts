...
export default class MetasController {
  public async update({ params, request, response }) {
    const userMeta = await UserMeta.query().where('user_id', parseInt(params.id)).firstOrFail();
  ...
  public async show({ params }) {
    let userMeta = await UserMeta.query().where('user_id', parseInt(params.id)).first();
    if (!userMeta) {
