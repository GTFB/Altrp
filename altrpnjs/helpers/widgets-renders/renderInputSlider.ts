...
export default function renderInputSlider(settings, device) {
  ...
  return `<div value="${min}" max="${max}" class="${"altrp-field-slider-wrapper" + (vertical ? " altrp-field-slider-vertical" : " altrp-field-slider-horizontal")}">
         ...
         <div class="bp3-slider-track">
           ...
           <div class="bp3-slider-axis">
              ${sliderLabels.map(step => {
                 return (
                   `<div class="bp3-slider-label" style="${toggleStyle + formatPercentage((step - min) / (max - min)) + ";"}">value: ${step}</div>`
                 )
              }).join(" ")}
           </div>
           <span class="bp3-slider-handle" tabindex="0" style="${toggleStyle + "calc(0% - 8px);"}">
