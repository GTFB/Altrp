...
export default class PageGenerator extends BaseGenerator {
  ...
  async run(page: Page) {
    ...
    plugin_frontend_head = await applyPluginsFiltersAsync(
    ...
    plugin_frontend_bottom = await applyPluginsFiltersAsync(
...
<style id="extra_header_styles">${content}</style>`;
    ...
    let global_styles_editor: AltrpMeta | string | null = await AltrpMeta.query()
      .where('meta_name', 'global_styles_editor')
      .first();
