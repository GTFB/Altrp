...
export default function renderImage(settings, device, context, widgetId) {
  ...
  if (getContent(settings, context, 'raw_url', device)) {
    ...
    getContent(settings, context, 'default_url', device) &&
    _.isString(_.get(context, getContent(settings, context, 'default_url', device), null))
  ) {
    media = {
      assetType: 'media',
  ...
  let altrpImage = AltrpImage(
    { image: media, widgetId, width, height, settings, class: 'altrp-image' },
    device
  );
  ...
  if (link.toPrevPage) {
    ...
    return `<div class='${classNames}'>${
      linkUrl
        ? `<a href='${linkUrl}' ${objectToAttributesString(linkProps)}>${altrpImage}</a>`
        : altrpImage
    }</div>`;
  }
}
