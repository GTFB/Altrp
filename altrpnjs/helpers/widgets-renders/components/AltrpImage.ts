...
export default function AltrpImage(props: any, device: string) {
  ...
  if (lazyload_disable) {
    if (image.rawSVG) {
      ...
      return `<svg xmlns="http://www.w3.org/2000/" class="altrp-image" ${objectToAttributesString(
        _props
      )}>${svg}</svg>`;
    }
  ...
  let placeholderStyles: any = {
    ...
    maxWidth: '100%',
    overflow: 'hidden',
    width: _.isNumber(props.width) ? props.width + 'px' : props.width ? props.width : '100%',
  };
  if (props.height) {
  ...
  const altrpSkeletonHighlightColor = get_altrp_setting(
    'altrp_skeleton_highlight_color',
    '#d0d0d0'
  );
