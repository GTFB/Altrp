...
export default class StartNode extends React.Component {
  ...
  changeSelectCustomizer = (e, type, key = false, item = false) => {
    let customizer = this.props.customizer;
    const value = e.target.value;

    if (type === "start_config") {
      if (key === 'period') {
        customizer.start_config.period.name = value;
      }
      if (key === 'restrictions') {
        if (_.isArray(customizer.start_config.restrictions)) {
          customizer.start_config.restrictions.map(i => {
      ...
      if (type === "start_condition") {
        switch (value){
          case "cron":
            ...
            break;
          case "telegram_bot":
            customizer.start_config = { bot_token: '' }
            break;
          default:
            customizer.start_config = '';
            break;
...
// Запись значений select в store
  changeInputCustomizer(e, type, key = false, id = false) {
    let customizer = this.props.customizer;
    let value = e.target.value;

    if (type === "bot_token") customizer.start_config.bot_token = value;
    if (type === "period") customizer.start_config.period.time = value;
    if (type === 'restrictions') {
      if (_.isArray(customizer.start_config.restrictions)) {
        customizer.start_config.restrictions.map(item => {
  ...
  onCreateCustomizer() {
    ...
    if (_.isArray(customizer.start_config.restrictions)) {
      customizer.start_config.restrictions.push(this.getNewRestriction());
    }
    store.dispatch(setCurrentCustomizer(customizer));
  }
  ...
  onDeleteCustomizer(item) {
    const restrictions = this.props.customizer?.start_config?.restrictions ?? [];
    const newRestrictions = restrictions.filter(i => i.id !== item.id);
    const customizer = this.props.customizer;
    customizer.start_config.restrictions = newRestrictions;
    store.dispatch(setCurrentCustomizer(customizer));
  }
  ...
  render() {
    ...
    const model = this.props.customizer?.model_id ?? '';
    const start = this.props.customizer?.start_condition ?? '';
    const periodOptions = [
    ...
    const botToken = this.props.customizer?.start_config?.bot_token ?? '';
    const period = this.props.customizer?.start_config?.period?.name ?? '';
    const periodTime = this.props.customizer?.start_config?.period?.time ?? '';
    const restrictions = this.props.customizer?.start_config?.restrictions ?? [];
            ...
            <div className="controller-container controller-container_select">
              ...
              <div className="control-container_select-wrapper controller-field">
                <select className="control-select control-field"
                        value={model || ''}
                        onChange={e => {
                          this.changeSelectCustomizer(e, "model_id")
                        }}
                >
                  <option value=""/>
            ...
            <div className="controller-container controller-container_select">
              ...
              <div className="control-container_select-wrapper controller-field">
                <select className="control-select control-field"
                        value={start || ''}
                        onChange={e => {
                          this.changeSelectCustomizer(e, "start_condition")
                        }}
                >
            ...
            {(start === 'telegram_bot') && <div className="controller-container controller-container_select">
                ...
                <div className="control-container_select-wrapper controller-field">
                  <input
                    ...
                    value={botToken || ''}
                    onChange={(e) => {
                      this.changeInputCustomizer(e, 'bot_token')
                    }}
                  />
                </div>
              </div>}
              ...
              <div className="controller-container controller-container_select">
                ...
                <div className="control-container_select-wrapper controller-field">
                  <select className="control-select control-field"
                          value={period || ''}
                          onChange={e => {
                            this.changeSelectCustomizer(e, "start_config", "period")
                          }}
                  >
                    <option disabled value=""/>
              ...
              {(period === "dailyAt") && <div className="controller-container controller-container_select">
                ...
                <div className="control-container_select-wrapper controller-field">
                  <input
                    ...
                    value={periodTime || ''}
                    onChange={(e) => {
                      this.changeInputCustomizer(e, 'period')
                    }}
                  />
                </div>
              </div>}
              ...
              <div className="controller-container controller-container_repeater repeater">
                  ...
                  {restrictions.map((item, index) =>
                    <div className="repeater-item repeater-item_open" key={index}>
                      <div className="repeater-item-tools">
                        ...
                        <button className="repeater-item__icon" onClick={() => this.onDeleteCustomizer(item)}>
                          {iconsManager().renderIcon('times')}
                        </button>
                      </div>
                      <div className="repeater-item-content">
                        <div className="controller-container controller-container_select2" style={{fontSize: '13px'}}>
                          <select className="control-select control-field"
                                  value={item?.name || ''}
                                  onChange={e => {
                                    this.changeSelectCustomizer(e, "start_config", "restrictions", item)
                                  }}
                          >
                            <option disabled value=""/>
                        ...
                        {(item.name === 'between') && <div className="ontroller-container controller-container_select">
                          ...
                          <div className="control-container_select-wrapper controller-field">
                            <input
                              ...
                              onChange={(e) => {
                                this.changeInputCustomizer(e, 'restrictions', 'start', item.id)
                              }}
                            />
                          </div>
                          <div className="controller-container__label control-select__label controller-label">
                          ...
                          <div className="control-container_select-wrapper controller-field">
                            <input
                              ...
                              value={item?.time?.end ?? ''}
                              onChange={(e) => {
                                this.changeInputCustomizer(e, 'restrictions', 'end', item.id)
                              }}
                            />
                          </div>
                        </div>}
                        {(item.name === 'when') && <div className="ontroller-container controller-container_select">
                          ...
                          <div className="control-container_select-wrapper controller-field">
                            <textarea
                              ...
                              onChange={(e) => {
                                this.changeInputCustomizer(e, 'restrictions', 'condition', item.id)
                              }}
                            />
                          </div>
                        </div>}
                ...
                <div className="d-flex justify-center repeater-bottom">
                  <button
                    className="btn font_montserrat font_500 btn_grey btn_active"
                    style={{background: '#87CA00', padding: '5px', fontSize: '10px', textTransform: 'uppercase'}}
                    onClick={() => this.onCreateCustomizer()}
                  >
                    Add
                  </button>
                </div>
