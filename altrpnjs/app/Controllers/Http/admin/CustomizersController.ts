...
export default class CustomizersController {
  public async store({ request, response }: HttpContextContract) {
    ...
    return response.json({
      success: true,
      data: customizer,
      redirect_route: '/admin/customizers-editor?customizer_id=' + customizer.id,
    });
  }
  ...
  public async update({ params, request, response }: HttpContextContract) {
    ...
    const oldSource = await Source.query()
      .where('sourceable_id', params.id)
    ...
    try {
      ...
      if (customizer?.settings?.time && customizer.settings?.time_type) {
        ...
        oldSettings?.time_type !== customizer?.settings?.time_type ||
        oldSettings?.time !== customizer?.settings?.time
      ) {
  ...
  public async index({ request, response }: HttpContextContract) {
    ...
    if (search) {
      customizers.where(function (query) {
        query
          .where('altrp_customizers.title', 'like', '%' + search + '%')
  ...
  public async destroy({ params, response }: HttpContextContract) {
    ...
    try {
      ...
      return response.json({
        ...
        'throw message': e.message,
        'trace': e.stack.split('\n'),
        'message': 'Customizer could not be deleted',
      });
    }
