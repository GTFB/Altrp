...
export default function renderInputRadio(settings, device, context) {
  ...
  const renderRepeatedInput = () => {
    ...
    return `<div class="altrp-field-subgroup"><div class="altrp-field-radio-group">${options
      .map((option, idx) => {
        let checked = false;
        ...
        return `<label key="${fieldName + '-' + idx}" class="${
          ...
          formID + '-' + fieldName
        }" type="radio"><span class="bp3-control-indicator"></span>${option.label}</label>`;
      })
      .join(' ')}</div></div>`;
  };
