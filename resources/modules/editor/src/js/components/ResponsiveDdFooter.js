        ...
        <div className={"responsive-footer-submenu " + (this.state.open && "responsive-footer-submenu-open")} >
          {
            [...this.state.screens].reverse().map(screen => {
              return <div className={"responsive-footer-submenu__item " + (this.props.currentScreen.id === screen.id && "responsive-footer-submenu__item__active")} onClick={() => this.setCurrentScreen(screen)} key={screen.id}>{iconsManager().renderIcon(screen.icon)}</div>
            })
          }
        </div>
      </div>
