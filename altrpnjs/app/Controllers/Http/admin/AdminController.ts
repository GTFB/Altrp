...
export default class AdminController {
  ...
  private static async upgradeTemplates(request: RequestContract) {
    ...
    if (id) {
      if (id.indexOf(',')) {
        id = id.split(',');
        templates = await Template.query().whereIn('id', id).select('*');
      } else {
        templates = await Template.query().where('id', id).select('*');
      }
    } else {
  ...
  private static async upgradePages(request: RequestContract) {
    ...
    if (id) {
      if (id.indexOf(',')) {
        id = id.split(',');
        pages = await Page.query().whereNull('deleted_at').whereIn('id', id).select('*');
      } else {
        pages = await Page.query().where('id', id).select('*');
      }
    } else {
