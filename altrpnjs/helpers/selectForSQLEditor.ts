      ...
      for (let key in _filters) {
        if (_filters.hasOwnProperty(key)) {
          ...
          if (_detail_filter_params[key]) {
            ...
            _detail_filter_conditionals.push(
              ' `' + _detail_filter_params[key] + '` LIKE "%' + value + '%" '
            );
          }
        }
      }
      ...
      for (let key in _filters) {
        if (_filters.hasOwnProperty(key)) {
          ...
          if (_detail_and_filter_params[key]) {
            ...
            _detail_and_filter_conditionals.push(
              ' `' + _detail_and_filter_params[key] + '` LIKE "%' + value + '%" '
            );
          }
        }
      }
  ...
  if (qs.order && qs.order_by) {
    ...
    if (sql.indexOf('ALTRP_DETAIL_FILTERS') !== -1) {
      ...
      if (_detail_filter_params[qs.order_by]) {
        _sql_order_by =
          ' ORDER BY ' +
          _detail_filter_params[qs.order_by] +
          '' +
          (qs.order === 'DESC' ? ' DESC' : ' ');
      }
      ...
      if (_detail_and_filter_params[qs.order_by]) {
        _sql_order_by =
          ' ORDER BY ' +
          _detail_and_filter_params[qs.order_by] +
          '' +
          (qs.order === 'DESC' ? ' DESC' : ' ');
      }
    }
