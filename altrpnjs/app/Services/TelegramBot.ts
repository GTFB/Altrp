...
export class TelegramBot {
  ...
  constructor(
    ...
    keyboard,
    updated = '',
    httpContext: null | HttpContextContract = null
  ) {
    if (httpContext) {
  ...
  async init(updated) {
    ...
    const customizer = await Customizer.query()
      .where('guid', this.webhook)
      .preload('altrp_model')
      .first();
    ...
    if (customizer) {
      ...
      if (global.telegramBot && updated) {
        if (
          global.telegramBot.polling !== undefined ||
          global.telegramBot.webhookServer !== undefined
        ) {
...
export default new TelegramBot(
  Env.get('ALTRP_SETTING_TELEGRAM_BOT_TOKEN'),
  Env.get('ALTRP_SETTING_TELEGRAM_BOT_WEBHOOK'),
  get_altrp_setting('telegram_bot_keyboard', '', true),
  '',
  null
);
