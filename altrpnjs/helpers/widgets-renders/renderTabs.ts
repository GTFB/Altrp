...
export default function renderTabs(settings, device, context) {
  ...
  let tabs = [];
  const spacing_icon_style = getResponsiveSetting(settings, 'spacing_icon_style', device) || {
    size: '10',
    unit: 'px',
  };
  if (settings.items_tabs) {
    tabs = settings.items_tabs?.map((tab, idx) => {
      let iconStyles = {};
      const alignment_icon_style =
        getResponsiveSetting(settings, 'alignment_icon_style', device) || 'left';
      if (alignment_icon_style === 'left') {
        iconStyles = {
          marginRight: spacing_icon_style?.size + spacing_icon_style?.unit,
        };
      } else {
        iconStyles = {
          marginLeft: spacing_icon_style?.size + spacing_icon_style?.unit,
        };
      }
  ...
  return `
    <div class="bp3-tabs altrp-tabs ${vertical ? 'altrp-tabs-vertical' : 'altrp-tabs-horizontal'} ${
