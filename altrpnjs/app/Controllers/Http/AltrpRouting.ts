...
export default class AltrpRouting {
  ...
  public async index(httpContext: HttpContextContract) {
    ...
    const modulesUrl =
      httpContext.request.protocol() + '://' + httpContext.request.host() + '/modules';
    ...
    let pageMatch: any = {};
    const page: Page | undefined = (await Page.query().whereNull('deleted_at').select('*')).find(
      (page) => {
    ...
    if (page) {
      ...
      if (page.model) {
        try {
          ...
          if (page.param_name && page.model_column && pageMatch?.params[page.param_name]) {
            classInstance = await ModelClass.where(
              page.model_column,
              pageMatch.params[page.param_name]
            );
      ...
      try {
        ...
        let res = await httpContext.view.render(
          `altrp/pages/${page.guid}`,
          Edge({
            ...altrpContext,
            hAltrp:
              Env.get('PATH_ENV') === 'production'
                ? '/modules/front-app/h-altrp.js'
                : 'http://localhost:3001/src/bundle.h-altrp.js',
            url:
              Env.get('PATH_ENV') === 'production'
                ? '/modules/front-app/front-app.js'
                : 'http://localhost:3001/src/bundle.front-app.js',
        ...
        return httpContext.response
          .send(`500 Internal Server Error to View Custom Page: ${e.message}
         ${e.stack}
         `);
      }
      /*
