...
export default class Page extends BaseModel {
  ...
  static FRONT_DEFAULT_AREAS = ['content', 'footer', 'header', 'popups'];
  ...
  public async getAreas(deleteContent = false) {
    ...
    let _contentAreas = await Area.query()
      .whereNotIn('name', ['card', 'popup', 'reports', 'email'])
      .select('*');
  ...
  public async attachRoles(roles) {
    ...
    for (const role_id of roles) {
      const duplicate = await PageRole.query()
  ...
  async getPageSettings(altrpRouting: AltrpRouting): Promise<object> {
      ...
      for (let template of templates) {
        {
          ...
          if (root_element) {
            recurseMapElements(root_element, function (element) {
              ...
              if (
                data_get(element, 'settings.tooltip_enable') &&
    ...
    altrpSettings.altrpMenus = (
      await Promise.all(
        altrpSettings.altrpMenus.map(async (menuGuid) => {
          let menu;
  ...
  static getRouteStyles(areas) {
    ...
    let columnsGrid = '';
    let rightSidebar = areas.find(
      (area) =>
        area.getSetting('area_type') === 'sidebar' &&
        area.getSetting('sidebar_location') === 'right'
    );
    let leftSidebar = areas.find(
      (area) =>
        area.getSetting('area_type') === 'sidebar' && area.getSetting('sidebar_location') === 'left'
    );
    columnsGrid += leftSidebar ? `${leftSidebar.getSetting('sidebar_width')}` : '0px';
    columnsGrid += ` calc(100% - ${
      leftSidebar ? `${leftSidebar.getSetting('sidebar_width')}` : '0px'
    } - ${rightSidebar ? `${rightSidebar.getSetting('sidebar_width')}` : '0px'}) `;
    ...
    styles += `grid-template-columns:${columnsGrid};grid-template-areas:
        '${
          (leftSidebar && leftSidebar.getSetting('sidebar_type')) === 'app_sidebar'
            ? 'left-sidebar'
            : 'header'
        } header ${
      (rightSidebar && rightSidebar.getSetting('sidebar_type')) === 'app_sidebar'
        ? 'right-sidebar'
        : 'header'
    }'
        '${contentRow}'
    ...
    styles +=
      '.altrp-section.altrp-section--full-width, .altrp-section.altrp-section--boxed{max-width:100%;}  .sections-wrapper{max-width: 100%;width: 100%;}';
    if (rightSidebar) {
      styles += rightSidebar.getCustomCSS();
      if (rightSidebar.getSetting('sidebar_fixed')) {
        styles += `.app-area_sidebar-location-right .sections-wrapper.sections-wrapper.sections-wrapper{overflow:hidden;position:fixed;top:0;right:0;width:${rightSidebar.getSetting(
          'sidebar_width',
          '0px'
        )};}`;
      }
    ...
    if (leftSidebar) {
      ...
      if (leftSidebar.getSetting('sidebar_fixed')) {
        styles += `.app-area_sidebar-location-left .sections-wrapper.sections-wrapper.sections-wrapper{overflow:hidden;position:fixed;top:0;left:0;width:${leftSidebar.getSetting(
          'sidebar_width',
          '0px'
        )};}`;
      }
  ...
  async getAllStyles() {
    ...
    let areas = await Area.query()
      .whereNotIn('name', ['content', 'header', 'footer', 'card', 'popup', 'reports', 'email'])
      .select('name');
...
<style id="altrp-generated-custom-areas-styles">${Page.getRouteStyles(contentAreas)}</style>`;
  ...
  async getChildrenContent() {
    ...
    let result = `<div class="app-area app-area_header">
      ...
      ${
        footerGuid
          ? `<link href="/altrp/css/${footerGuid}.css?${footerHash}" id="altrp-footer-css-link-${footerGuid}" rel="stylesheet"/>`
          : ''
      }
      `;
  ...
  async _extractElementsNames(element, elementNames, only_react_elements) {
    ...
    if (
      !(
        only_react_elements &&
        !(data_get(element, 'settings.react_element') || reactElements.indexOf(element.name) !== -1)
      )
    ) {
    ...
    if (element.name === 'template' && data_get(element, 'settings.template')) {
      await this.extractElementsNamesFromTemplate(
        data_get(element, 'settings.template'),
        elementNames
      );
    }
    if (element.name === 'posts' && data_get(element, 'settings.posts_card_template')) {
      await this.extractElementsNamesFromTemplate(
        data_get(element, 'settings.posts_card_template'),
        elementNames
      );
    }
    ...
    if (element.name === 'dropbar' && data_get(element, 'settings.template_dropbar_section')) {
      await this.extractElementsNamesFromTemplate(
        data_get(element, 'settings.template_dropbar_section'),
        elementNames
      );
    }
    if (element.name === 'carousel' && data_get(element, 'settings.slides_repeater').length > 0) {
    ...
    if (element.name === 'table' && data_get(element, 'settings.tables_columns')) {
      let columns = data_get(element, 'settings.tables_columns', []);
      for (let column of columns) {
        if (data_get(column, 'column_template')) {
          await this.extractElementsNamesFromTemplate(
            data_get(column, 'column_template'),
            elementNames
          );
        }
      }
    }
    if (element.name === 'tabs' && data_get(element, 'settings.items_tabs')) {
