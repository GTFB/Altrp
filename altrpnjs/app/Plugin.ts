...
export default class Plugin {
    ...
    plugins.forEach((plugin) => {
      try {
        if (plugin.getMeta(Plugin.PLUGIN_WIDGETS)) {
          new_widget_list = _.concat(
            new_widget_list,
            plugin.getMeta(Plugin.PLUGIN_WIDGETS).split(',')
          );
        }
      } catch (e) {}
    });
  ...
  private getMeta(meta_name, _default: any = null) {
    ...
    if (is_null(this.plugin_meta_data)) {
      ...
      this.plugin_main_file_content = fs.readFileSync(this.getPath('/plugin_meta'), {
        encoding: 'utf8',
      });
  ...
  async getAndCallHooks(folderName: string) {
    ...
    if (hasType) {
      ...
      for (const hookName of hookNames) {
        ...
        try {
          const hook = isProd() ? require(filePath).default : (await import(filePath)).default;
          hooks.push({
            fn: hook,
            hookName,
          });
